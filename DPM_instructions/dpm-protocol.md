...
Agent rules for this sample project live under `DPM_instructions/` and your agent's config directory (e.g. `.agent/` or `.cursor/`).

- Acquire/check `memory/.dpm_lock` at session start, and delete it at session end to release the lock.

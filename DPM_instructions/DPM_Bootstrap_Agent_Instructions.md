...
- **Other statuses**: Ask the user what to do with files that are not `NEW` or `Current`.

#### `/scanandingest`
Run `/scanrepo`, prompt the user about missing files and files with other statuses, ingest safe `NEW` and `Stale` files, and pause for duplicates, unsupported formats, manifest/index drift, conflicting facts, or likely superseded files. Report processed files, skipped files, conflicts, and memory sections updated.

...
3. **Prompt the user** for direction on **Missing files** ("Should I delete this file from memory, or are you going to re-add it?") and files with **Other statuses** (non-`NEW` or `Current`).
4. Flag manifest/index drift.
5. Recommend the smallest next action, such as `/ingestfile [path]` or `/scanandingest`.
...
1. Run `/scanrepo`.
2. **Prompt the user** for direction on **Missing files** and files with **Other statuses**.
3. Ingest safe `NEW` and `Stale` files.
4. Pause for duplicates, unsupported formats, manifest/index drift, conflicting source facts, or likely superseded files.
5. Produce a final report listing processed files, skipped files, conflicts, and memory sections updated.
